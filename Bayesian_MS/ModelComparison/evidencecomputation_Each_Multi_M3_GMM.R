...
GM_data <- readRDS(file="GaussianMixtures/ReparammvpdfPost_ALL_Long_Model3.stan.rds")
MU <- GM_data$parameters$mean
E <- GM_data$parameters$variance$sigma
w <- GM_data$parameters$pro

# Select initial point for the optimisation as the Mean, Cov of the Gaussian Mixture component with the highest weight
index <-which.max(w)
init<- MU[,index]
...
hessdet <- c()
Post_MAP_vect <-c()
theta_MAP_matrix <- matrix(data = 0, nrow = length(w),ncol = 14)
for(i in 1:length(w)){
...
for(e in 1:length(FileNames)){ # iterate over the experiments
   # Load the results of the gaussian mixture approximation
    ...
    Simul <- function(exper, param){
      library(rstan)
      expose_stan_functions("Simulations/Model3_Function_FakeData.stan")
    ...
    LogPriorTerm <-function(theta_MAP){
      ...
      pr[2] = dlnorm(theta_MAP[2], meanlog = -2.30258509299405, sdlog = 1.15129254649702)
      pr[3] =  dlnorm(theta_MAP[3], meanlog = -3.50655789731998, sdlog = 1.15129254649702)
      pr[4] = dlnorm(theta_MAP[4],meanlog = 2.30258509299405,sdlog = 1.15129254649702)
      pr[5] = dlnorm(theta_MAP[5],meanlog = 3.40119738166216, sdlog = 1.15129254649702)
      pr[6] = dlnorm(theta_MAP[6],meanlog=2.30258509299405,sdlog = 1.15129254649702)
      pr[7] = dnorm(theta_MAP[7],mean = 2.5,sd = 1.25);
      pr[8] = dnorm(theta_MAP[8],mean = 2.5,sd = 1.25);
      pr[9] = dlnorm(theta_MAP[9],meanlog=2.30258509299405,sdlog = 1.15129254649702)
      pr[10] = dlnorm(theta_MAP[10],meanlog=2.22044604925031e-16,sdlog = 1.15129254649702)
      pr[11] = dlnorm(theta_MAP[11],meanlog=3.40119738166216,sdlog = 1.15129254649702)
      pr[12] = dlnorm(theta_MAP[12],meanlog=-2.30258509299405,sdlog = 1.15129254649702)
      pr[13] = dnorm(theta_MAP[13],mean = 2.5,sd = 1.25);
      pr[14] = dnorm(theta_MAP[14],mean = 2.5,sd = 1.25);
...
# Would it be better to compute the Hessian using the approximation of the posterior?
    ...
    l3_single<- c(l3_single,l)
}

# Save results as a CSV file
# tit <- paste("lnEvidenceModel3_GMM.csv", sep = "")
# 
# write.table(t(l3), file = tit, col.names =c("Calibration_4","Calibration_5","Calibration_6","DynStim_1","DynStim_2", "DynStim_3","DynStim_8", "DynStim_9","DynStim_11", "DynStim_14"), 
#             row.names = "lnEvidenceModel3_GMM", sep=",")
# 


# tit <- paste("DetHessianModel3_GMM_.csv", sep = "")
# 
# write.table(t(hdet), file = tit, col.names =FileNames, 
#             row.names = "DetHessianModel3_GMM", sep=",")
