...
GM_data <- readRDS(file="GaussianMixtures/ReparammvpdfPost_ALL_Model4.stan.rds")
...
for(e in 1:length(FileNames)){ # iterate over the experiments
   # Load the results of the gaussian mixture approximation
    ...
    Simul <- function(exper, param){
      library(rstan)
      expose_stan_functions("Simulations/Model4_Function_FakeData.stan")
...
# Would it be better to compute the Hessian using the approximation of the posterior?
    ...
    l4_single<- c(l4_single,l)
}

# Save results as a CSV file
 tit <- paste("lnEvidenceModel4_GMM_Each_Multi.csv", sep = "")
 
 write.table(t(l4_single), file = tit, col.names =c("Calibration_4","Calibration_5","Calibration_6","DynStim_1","DynStim_2", "DynStim_3","DynStim_8", "DynStim_9","DynStim_11", "DynStim_14"), 
            row.names = "lnEvidenceModel4_GMM", sep=",")
